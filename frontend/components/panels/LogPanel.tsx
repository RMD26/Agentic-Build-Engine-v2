...
import { Box, LayoutPanelLeft, Server, Terminal as TerminalIcon, Trash2 } from 'lucide-react';
import { LogPanelProps, LogPanelEntry } from './panelTypes';

// ─── Log line ─────────────────────────────────────────────────────────────────

const LogLine: React.FC<{ log: LogPanelEntry }> = ({ log }) => {
...
type TabId = (typeof SOURCE_TABS)[number]['id'];
 ...
 * LogPanel
 *
 * A self-contained, filterable log display panel. Accepts an array of
 * `LogPanelEntry` items and an optional `onClear` callback. Designed to be
 * composed inside any layout — it does not pull from any global store itself.
 */
export const LogPanel: React.FC<LogPanelProps> = ({
  ...
  const filteredLogs =
    activeTab === 'all' ? logs : logs.filter((l) => l.source === activeTab);
  ...
  return (
    <div
      className={[
        'h-64 border-t border-border bg-[#050505] flex flex-col shrink-0 font-mono text-[11px] sm:text-xs',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
          ...
          <div className="flex items-center gap-1">
            {SOURCE_TABS.map(({ id, label, Icon }) => (
              <button
                ...
                className={[
                  'flex items-center gap-2 px-3 py-2 text-xs font-sans font-medium border-b-2 transition-colors',
                  activeTab === id
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-1">
        {filteredLogs.length === 0 ? (
          <div className="text-muted-foreground italic px-2">
            No logs to display for this source.
          </div>
        ) : (
